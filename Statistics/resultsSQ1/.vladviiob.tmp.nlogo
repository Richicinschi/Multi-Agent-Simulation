...
to setup
  ...
  ; Add this check before infection
  if behaviorspace-run-number > 0 and selected-country != "" [
    user-message (word "Starting infection in: " selected-country)
    infect-initial-agents-in-country selected-country
    stop  ; Stop here to prevent the ifelse below from running
  ]

  ; Original infection logic for non-BehaviorSpace runs
...
to prompt-for-country
  ; Check if running from BehaviorSpace
  ifelse behaviorspace-run-number > 0 [
    ; If in BehaviorSpace, use the selected-country directly
    if selected-country != "" [
      infect-initial-agents-in-country selected-country
    ]
  ][
    ; If not in BehaviorSpace, use the interactive prompt
...
to export-statistics
  ; Original detailed per-tick export
  let detailed-filename (word
    "data_"
    "hr" virus-heat-resistance "_"
    "cr" virus-cold-resistance "_"
    "tp" airports-and-ports-enabled? "_"
    "is" isolation-enabled? "_"
    "ap" num-airports "_"
    "run" behaviorspace-run-number
    ".csv")

  ifelse ticks = 0 [
    carefully [ file-delete detailed-filename ][]
    file-open detailed-filename
    file-type (word
      "tick,"
      ; General statistics
  ...
  ; Export summary data at end of run or key events
  if ticks = 0 or total-infected = 0 or ticks >= 365 [
    export-summary-data
  ]

  ; Export plots at regular intervals
  if ticks mod 30 = 0 or total-infected = 0 or ticks >= 365 [
    export-plots
  ]
end

to export-summary-data
  let summary-filename (word
    behaviorspace-experiment-name "_summary.csv")

  ifelse not file-exists? summary-filename [
    ; Create new file with headers if it doesn't exist
    file-open summary-filename
    file-type (word
      "experiment,run,configuration,"
      ; Peak metrics
      "peak_infected,peak_infected_tick,time_to_peak,"
      ; Final state
      "final_infected,final_recovered,final_dead,"
      ; Regional impact
      "max_hot_transmission,max_cold_transmission,max_moderate_transmission,"
      ; Transport totals
      "total_airport_usage,total_port_usage,"
      ; Disease spread
      "max_affected_countries,total_cross_border,total_within_country\n")
  ][
    file-open summary-filename
  ]

  ; Calculate summary statistics
  let configuration (word
    "hr" virus-heat-resistance "_cr" virus-cold-resistance "_tp"
    airports-and-ports-enabled? "_is" isolation-enabled? "_ap" num-airports)
  ...
  file-type (word
    ...
    configuration ","
    ; Peak metrics
    max [total-infected] of turtles ","
    ticks "," ; tick at which peak occurred
    ticks "," ; time to peak
    ; Final state
    total-infected "," total-recovered "," total-dead ","
    ; Regional impact
    calculate-transmission-rate heat-threshold "hot" ","
    calculate-transmission-rate cold-threshold "cold" ","
    calculate-transmission-rate 0 "moderate" ","
    ; Transport
    airport-usage-count ","
    port-usage-count ","
    ; Disease spread
    length remove-duplicates [country-name] of turtles with [status = "infected"] ","
    count-cross-border-infections ","
    count-within-country-infections
    "\n")
...
to export-plots
  foreach ["Disease Progression" "Daily New Cases" "Infection Rate" "Climate Impact"] [ plot-identifier ->
    let plot-filename (word
      behaviorspace-experiment-name "_"
      behaviorspace-run-number "_"
      (remove " " plot-identifier) "_"
      ticks ".csv")

    carefully [
      export-plot plot-identifier plot-filename
    ][
      print (word "Failed to export plot: " plot-identifier)
    ]
  ]
end
...
; Original helper procedures unchanged
...
infect-initial-agents-in-country selected-country</setup>
  ...
  <experiment name="Transportation_Study" repetitions="10" runMetricsEveryStep="false">
    <setup>setup
set select-infection-country? true
set selected-country "Romania"</setup>
    <go>go</go>
    <timeLimit steps="366"/>
    <exitCondition>total-infected = 0 or ticks &gt;= 366</exitCondition>
    <enumeratedValueSet variable="airports-and-ports-enabled?">
